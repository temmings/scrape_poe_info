...
def convert_data_to_AHK_readable_format(all_data):
    ...
    :return:
    """
    new_data = write_file_headers()
    for line in all_data:  # this is the list which contains the item's data
        line_hold = ""
        count = 0
        for data in line:  # this iters thru the list and uses regex to search each string
    ...
    file = open('Uniques.txt', 'w')  # opens file for writing
    for row in new_data:
        file.write(row + '\n')  # write each line
    file.close()
...
data = parse_category_data(links_list)
convert_data_to_AHK_readable_format(data)
print('Done')
